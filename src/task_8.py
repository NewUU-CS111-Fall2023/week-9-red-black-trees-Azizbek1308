...
class RedBlackTree:
    ...
    def search_helper(self, node, value):
        if node is None or node.data == value:
            if node is not None:
                return f"{value} is found, its height is {self.get_black_height(node)}"
            else:
    ...
    def median(self):
        ...
        if size % 2 == 0:
            ...
            median = (nodes[median_index] + nodes[median_index + 1]) / 2
        else:
            median_index = size // 2
            median = nodes[median_index]

        return f"The median is {int(median)}"
